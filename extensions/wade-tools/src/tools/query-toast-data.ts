...
export const definition = {
  ...
  description:
    "Query Toast POS / F&B data directly from structured tables. ALWAYS use this for restaurant revenue, food sales, labor hours, menu item performance, or F&B metrics. Do NOT use query_financial_data for F&B/Toast questions.",
  parameters: {
    type: "object",
    properties: {
      metric: {
...
export async function execute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  ...
  const restaurantGuid = params.restaurant_guid as string | undefined;
  ...
  if (metric === "daily_sales") {
    ...
    const daily = new Map<string, { date: string; revenue: number; orders: number; guests: number; tips: number }>();
    for (const row of rows) {
      const date = (row.business_date as string) ?? "unknown";
  ...
  if (metric === "labor") {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let q = (supabase.from("toast_time_entries") as any)
      .select("*")
      .order("business_date", { ascending: false })
      .limit(5000);
    ...
    const rows = (data ?? []) as Record<string, unknown>[];
    const daily = new Map<string, { date: string; regular_hours: number; overtime_hours: number; labor_cost: number; entries: number }>();
    for (const row of rows) {
      const date = (row.business_date as string) ?? "unknown";
  ...
  if (metric === "top_items") {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let q = (supabase.from("toast_order_items") as any)
      .select("*")
      .eq("voided", false)
      .limit(10000);
    ...
    const rows = (data ?? []) as Record<string, unknown>[];
    const items = new Map<string, { item_name: string; quantity: number; revenue: number }>();
    for (const row of rows) {
      const name = (row.item_name as string) ?? "(unknown)";
  ...
  if (metric === "category_sales") {
    ...
    const rows = (data ?? []) as Record<string, unknown>[];
    const cats = new Map<string, { category: string; quantity: number; revenue: number }>();
    for (const row of rows) {
      const cat = (row.sales_category as string) ?? "(uncategorized)";
  ...
  if (metric === "payment_mix") {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let q = (supabase.from("toast_payments") as any)
      .select("*")
      .limit(10000);
    ...
    const rows = (data ?? []) as Record<string, unknown>[];
    const types = new Map<string, { payment_type: string; count: number; total_amount: number }>();
    for (const row of rows) {
      const ptype = (row.payment_type as string) ?? "(unknown)";
