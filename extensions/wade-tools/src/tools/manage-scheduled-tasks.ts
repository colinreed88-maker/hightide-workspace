...
export const definition = {
  ...
  description:
    "Manage scheduled tasks (cron jobs). Actions: 'list' to see all tasks, 'create' to add a new one, 'update' to modify an existing one, 'delete' to remove one, 'enable'/'disable' to toggle. Tasks run automatically on the cron schedule.",
  parameters: {
    type: "object",
    properties: {
      action: {
...
export async function execute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  ...
  switch (action) {
    ...
    case "create": {
      ...
      const cronExpr = params.cron_expression as string;
      if (!name || !cronExpr) return textResult({ error: "name and cron_expression are required for create." });

      const { data, error } = await supabase.from("scheduled_tasks").insert({
        name,
        cron_expression: cronExpr,
        timezone: (params.timezone as string) ?? "America/Los_Angeles",
        job_type: (params.job_type as string) ?? "freeform_question",
        job_instructions: (params.job_instructions as string) ?? null,
        job_context: (params.job_context as Record<string, unknown>) ?? {},
        enabled: true,
      }).select().single();
    ...
    case "update": {
      ...
      if (!taskId) return textResult({ error: "task_id is required for update." });

      const updates: Record<string, unknown> = {};
      if (params.name) updates.name = params.name;
      if (params.cron_expression) updates.cron_expression = params.cron_expression;
      if (params.timezone) updates.timezone = params.timezone;
      if (params.job_type) updates.job_type = params.job_type;
      if (params.job_instructions !== undefined) updates.job_instructions = params.job_instructions;
      if (params.job_context) updates.job_context = params.job_context;
    ...
    case "delete": {
      const taskId = params.task_id as string;
      if (!taskId) return textResult({ error: "task_id is required for delete." });
    ...
    case "disable": {
      const taskId = params.task_id as string;
      if (!taskId) return textResult({ error: "task_id is required." });
