...
export const definition = {
  name: "search_web",
  description:
    "Search the internet for current information. Use for news, real-time data, market trends, company info, or anything not available in the knowledge base or financial tools. Returns summarized results from web pages.",
  parameters: {
    type: "object",
    properties: {
...
export async function execute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  ...
  if (!apiKey) return textResult({ error: "TAVILY_API_KEY not configured." });
