...
export async function execute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  ...
  if (!serviceKey) return textResult({ error: "ASSISTANT_SERVICE_KEY not configured" });
  ...
  try {
    ...
    const data = (await res.json()) as { response: string; tools_used: string[] };
