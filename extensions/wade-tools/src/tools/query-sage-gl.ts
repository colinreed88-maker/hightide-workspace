...
import {
  resolveBudgetDeptToSageDepts,
  resolveDeptNamesToIds,
  loadDeptIdToName,
  loadAccountNoToTitle,
  loadFsMapping,
  loadBuMapping,
...
export async function execute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  ...
  const accountNo = params.account_no as string | undefined;
  const dateFrom = params.date_from as string | undefined;
  const dateTo = params.date_to as string | undefined;
  const source = (params.source as string) ?? "monthly_dept_balances";
  ...
  if (source === "gl_detail") {
    ...
    if (effectiveDeptFilter && !departmentId) q = q.in("department_id", effectiveDeptFilter);
    if (entityId) q = q.eq("location_id", entityId);
    if (accountNo) q = q.eq("account_no", accountNo);
    ...
    const enriched = rows.map((r) => ({
      ...r,
      resolved_department: deptNameMap.get(r.department_id as string) ?? r.department_id,
      resolved_bu: buIdMap.get(r.department_id as string) ?? "(unmapped)",
    }));
  ...
  if (accountNo) q = q.eq("account_no", accountNo);
  if (dateFrom) q = q.gte("year_month", dateFrom.slice(0, 7));
  if (dateTo) q = q.lte("year_month", dateTo.slice(0, 7));
  ...
  const emptyFsMap = new Map<string, FsMappingEntry>();
  const [buIdMap, deptNameMap, acctNameMap, fsMap] = await Promise.all([
    loadBuMappingByDeptId(),
    loadDeptIdToName(),
    needAcctNames ? loadAccountNoToTitle() : Promise.resolve(new Map<string, string>()),
    needFsMapping ? loadFsMapping() : Promise.resolve(emptyFsMap),
  ]);

  interface AggRow { group_key: string; net_amount: number; debit_total: number; credit_total: number; line_count: number }
  ...
  for (const row of rows) {
    let key = row[col] as string ?? "(unknown)";
    if (groupBy === "business_unit") key = buIdMap.get(key) ?? "(unmapped)";
