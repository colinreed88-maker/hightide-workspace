...
export async function createCalendarEventExecute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  const supabase = getSupabase();
  ...
  if (error) return textResult({ error: `Failed to queue calendar event: ${error.message}` });
...
export async function getCalendarAvailabilityExecute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  try {
...
export async function listUpcomingEventsExecute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  try {
    const events = await listEvents((params.max_results as number) ?? 10);
