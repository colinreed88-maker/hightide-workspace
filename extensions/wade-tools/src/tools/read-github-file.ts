...
export const definition = {
  ...
  description:
    "Read a file from the Flow codebase on GitHub. Use to understand how pages work, how data is structured, what queries or filters a page uses, or how APIs are implemented. Available repos: flow-intranet, flow-ai-worker.",
  parameters: {
    type: "object",
    properties: {
      repo: {
...
export async function execute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  ...
  if (!token) return textResult({ error: "GITHUB_READ_TOKEN not configured." });
  ...
  try {
    ...
    if (Array.isArray(data)) {
      const listing = data.map((item: { name: string; type: string; size: number; path: string }) => ({
        name: item.name,
        type: item.type,
        size: item.size,
        path: item.path,
