...
export const definition = {
  ...
  description:
    "Query headcount and employee roster data directly from structured tables. ALWAYS use this for headcount questions, employee lookups, compensation queries, or department staffing. Do NOT use query_financial_data for headcount questions.",
  parameters: {
    type: "object",
    properties: {
      source: {
...
export async function execute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  ...
  const source = (params.source as string) ?? "current_roster";
  const businessUnit = params.business_unit as string | undefined;
  const department = params.department as string | undefined;
  const employeeSearch = params.employee_search as string | undefined;
  const status = params.status as string | undefined;
  const monthFrom = params.month_from as string | undefined;
  const monthTo = params.month_to as string | undefined;
  const groupBy = params.group_by as string | undefined;
  const limit = (params.limit as number) ?? 50;

  if (source === "current_roster") {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let q = (supabase.from("mbr_current_roster") as any).select("*").limit(limit);

    if (businessUnit) q = q.eq("Organization/BU", businessUnit);
    if (department) q = q.eq("Reporting_Dept", department);
    if (status) q = q.eq("Employment status", status);
    if (employeeSearch) q = q.ilike("Employee", `%${employeeSearch}%`);
    ...
    if (groupBy) {
      const colMap: Record<string, string> = { business_unit: "Organization/BU", department: "Reporting_Dept", location: "Work location name" };
  ...
  if (source === "fpa_headcount_monthly") {
    let scenarioId = params.scenario_id as string | undefined;
    if (!scenarioId) {
      const { data: scenarios } = await supabase
        ...
        .limit(1);
      scenarioId = (scenarios?.[0] as { id: string } | undefined)?.id;
      if (!scenarioId) return textResult({ error: "No active FPA scenario found." });
    }

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let q = (supabase.from("fpa_headcount_monthly") as any)
      .select("*")
  ...
  if (source === "fpa_employees") {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let q = (supabase.from("fpa_employees") as any)
      .select("*")
      .eq("scenario_id", scenarioId)
      .limit(limit);
    ...
    if (businessUnit) q = q.eq("business_unit", businessUnit);
    if (department) q = q.eq("department", department);
    if (employeeSearch) q = q.ilike("employee_name", `%${employeeSearch}%`);
