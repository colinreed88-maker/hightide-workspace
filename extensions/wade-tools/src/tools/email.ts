...
async function countTodayActions(actionType: string): Promise<number> {
...
async function getDailyLimit(actionType: string): Promise<number> {
  const supabase = getSupabase();
  ...
  if (overrides?.length) {
    const match = (overrides[0] as { content: string }).content.match(/override:\s*(\d+)/i);
...
async function queueForApproval(
  ...
  if (error) return textResult({ error: `Failed to queue ${toolName}: ${error.message}` });
...
export const sendEmailDef = {
  name: "send_email",
  description:
    "Send an email via Resend. Use for notifications, reports, and communications. Domain-restricted to allowed domains.",
  parameters: {
    type: "object",
    properties: {
...
export const approveActionDef = {
  name: "approve_action",
  description:
    "Approve a pending action (email or calendar event) after the user explicitly confirms. Only call this when the user has clearly approved the proposed action in the current conversation.",
  parameters: {
    type: "object",
    properties: {
      action_id: {
...
export async function approveActionExecute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  return handleActionDecision(params.action_id as string, "approved", _id);
}

export const rejectActionDef = {
  name: "reject_action",
  description:
    "Reject/cancel a pending action (email or calendar event) when the user declines.",
  parameters: {
    type: "object",
    properties: {
      action_id: {
...
export async function rejectActionExecute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  return handleActionDecision(params.action_id as string, "rejected", _id);
}
...
async function handleActionDecision(
  ...
  if ((action as { status: string }).status !== "proposed") {
    return textResult({ error: `Action is already ${(action as { status: string }).status}, cannot ${decision}.` });
  }
  ...
  const label = (action as { action_type: string }).action_type === "send_email" ? "email" : "calendar event";
