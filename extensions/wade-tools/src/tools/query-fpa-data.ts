...
export const definition = {
  name: "query_fpa_data",
  description:
    "Query FP&A forecast/budget data directly from structured tables. ALWAYS use this for budget vs actuals, forecast line items, department P&L, headcount forecasts, or KPIs. business_unit is directly on FPA rows. Do NOT use query_financial_data for budget/forecast questions.",
  parameters: {
    type: "object",
    properties: {
      table: {
...
export async function execute(_id: string, params: Record<string, unknown>): Promise<ToolResult> {
  ...
  let businessUnit = params.business_unit as string | undefined;
  const department = params.department as string | undefined;
  const lineItem = params.line_item as string | undefined;
  ...
  if (department && !businessUnit && deptHasBu) {
    ...
    if (mappedBu) businessUnit = mappedBu;
  }
  ...
  if (section && table === "fpa_pnl_monthly") q = q.eq("section", section);
  if (lineItem) q = q.ilike("line_item", `%${lineItem}%`);
  ...
  if ((data ?? []).length === 0 && department && businessUnit && deptHasBu) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let retryQ = (supabase.from(table) as any)
      .select("*")
      .eq("scenario_id", scenarioId)
      .eq("business_unit", businessUnit)
      .limit(limit);

    if (lineItem) retryQ = retryQ.ilike("line_item", `%${lineItem}%`);
    if (monthFrom && ["fpa_dept_pnl_monthly", "fpa_pnl_monthly", "fpa_headcount_monthly"].includes(table)) {
